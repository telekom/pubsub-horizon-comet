...
public class ThreadPoolBackpressureHandler implements RejectedExecutionHandler {
    ...
    public ThreadPoolBackpressureHandler(MeterRegistry meterRegistry, ThreadPoolTaskExecutor deliveryTaskExecutor,
        ...
        deliveryTaskExecutor.getThreadPoolExecutor().setRejectedExecutionHandler(this);
    ...
    private void onBackpressureEvent(BackpressureEvent event) {
        switch (event) {
            case PAUSE -> {
                ...
                log.info("ThreadPool Backpressure PAUSE: kafka listener container paused due to thread pool saturation");
            }
            case RESUME -> {
                messageListenerContainer.resume();
                log.info("ThreadPool Backpressure  RESUME - kafka listener container resumed");
            }
        }
    }
    ...
    public void rejectedExecution(Runnable runnable, ThreadPoolExecutor threadPoolExecutor) {
        ...
        log.info("Task rejected from BackpressureHandler");
